...
export async function execute(interaction) {
  try {
    ...
    console.error('[create-ticket-panel execute] Error:', err.message, err.stack);
    throw err;
  }
}
...
export async function handleModal(interaction) {
  ...
  // Must defer first — modal interactions are already acknowledged by Discord
  ...
  if (!staffRole) {
    return interaction.editReply({ content: `❌ Staff role not found. Check <@&${staffRoleId}> is a valid role.` });
  }
  if (!pingRole) {
    return interaction.editReply({ content: `❌ Ping role not found. Check <@&${pingRoleId}> is a valid role.` });
  }
  if (!category || category.type !== 4) {
    return interaction.editReply({ content: `❌ Category not found. Check that ${categoryId} is a valid category channel.` });
  }
