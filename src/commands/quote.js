...
export async function execute(interaction) {
  ...
  if (!m) {
    return interaction.reply({
      content: '❌ That doesn\'t look like a Discord message link. Right-click a message → Copy Message Link.',
      ephemeral: true,
    });
  }
  ...
  if (guildId !== '@me' && !interaction.client.guilds.cache.has(guildId)) {
    return interaction.editReply({ content: '❌ Bot is not in the source server.' });
  }

  const channel = await interaction.client.channels.fetch(channelId).catch(() => null);
  if (!channel || !channel.isTextBased?.()) {
    return interaction.editReply({ content: '❌ Couldn\'t access that channel.' });
  }

  const msg = await channel.messages.fetch(messageId).catch(() => null);
  if (!msg) {
    return interaction.editReply({ content: '❌ Message not found (deleted or no read history).' });
  }
