...
export async function execute(interaction) {
  ...
  const embed = new EmbedBuilder()
    .setTitle('Kudos!')
    .setColor(0xfacc15)
    .setDescription(`${E.kudos} <@${interaction.user.id}> wants to thank <@${target.id}>.`)
    .addFields(
      { name: 'From', value: `<@${interaction.user.id}>`, inline: true },
      { name: 'To', value: `<@${target.id}>`, inline: true },
      { name: 'For', value: `> ${reason.split('\n').join('\n> ')}`, inline: false },
    )
    .setFooter({
      text: recentCount === 1
      ...
      await target.send({
        embeds: [
          new EmbedBuilder()
            ...
            .setColor(0xfacc15)
            .setDescription(`${E.kudos} <@${interaction.user.id}> just sent you a public thanks.`)
            .addFields(
              { name: 'From', value: `<@${interaction.user.id}>`, inline: true },
              { name: 'For', value: `> ${reason.split('\n').join('\n> ')}`, inline: false },
            )
