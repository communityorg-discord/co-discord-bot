...
export async function execute(interaction) {
  try {
    ...
    const verifyStatus = verified
      ? `${E.check} Verified`
      : pendingQueue
      ? `${E.pending} Pending (#${pendingQueue.id})`
      : lastQueue?.status === 'denied'
      ? `${E.cross} Denied`
      : lastQueue?.status === 'unverified'
      ? `${E.cross} Unverified`
      : `${E.cross} Not Verified`;
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields(
        ...
        { name: 'Portal Account', value: portalUser ? `${E.check} Linked` : `${E.cross} Not linked`, inline: true },
        ...
        { name: 'Verified Nickname', value: verified?.nickname || 'N/A', inline: true },
        { name: 'Verified Position', value: verified?.position || 'N/A', inline: true },
        { name: 'Verified Since', value: verified?.verified_at ? `<t:${Math.floor(new Date(verified.verified_at).getTime() / 1000)}:D>` : 'N/A', inline: true },
        ...
        { name: 'Under Investigation', value: investigation ? `${E.warning} Yes` : `${E.check} No`, inline: true },
        { name: 'Suspended', value: suspension ? `${E.warning} Yes` : `${E.check} No`, inline: true },
